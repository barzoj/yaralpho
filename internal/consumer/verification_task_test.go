...
func TestVerificationTaskDelegatesToStructuredExecution(t *testing.T) {
	execTask := NewExecutionTask(&stubExecutionTracker{}, nil, nil, notify.Noop{}, zap.NewNop(), " /repo ", "exec prompt")
	...
	task := NewVerificationTask(execTask, " verify prompt ")
	...
	require.Equal(t, "verify prompt", capturedPrompt)
...
func TestVerificationTaskRequiresExecutionReference(t *testing.T) {
	task := NewVerificationTask(nil, "prompt")
