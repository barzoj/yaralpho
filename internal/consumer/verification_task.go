...
func (t *VerificationTask) Execute(ctx context.Context, batch *storage.Batch, taskID string) (storage.TaskRunStatus, string, error) {
	...
	return t.exec(ctx, t.executionTask.copilot, t.executionTask.storage, t.executionTask.tracker, notifier, logger, repoPath, newRunID, now, batch, taskID, prompt)
}
