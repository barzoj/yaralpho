...
func TestAgentCreateDefaultsIdle(t *testing.T) {
	...
	body := []byte(`{"name":"worker1","type":"codex"}`)
	...
	require.Equal(t, "codex", resp.Type)
...
func TestAgentCreateRejectsInvalidType(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/agent", bytes.NewBufferString(`{"name":"w","type":"invalid"}`))
	rec := httptest.NewRecorder()
	app.Router().ServeHTTP(rec, req)
...
func TestAgentListAndGet(t *testing.T) {
	st := newHandlerTestStorage()
	now := time.Now().UTC()
	st.agents["a1"] = storage.Agent{ID: "a1", Name: "one", Type: "codex", Status: storage.AgentStatusIdle, CreatedAt: now, UpdatedAt: now}
	st.agents["a2"] = storage.Agent{ID: "a2", Name: "two", Type: "copilot", Status: storage.AgentStatusBusy, CreatedAt: now, UpdatedAt: now}
	...
	require.Equal(t, "copilot", agent.Type)
}

func TestAgentUpdateBlocksWhenBusy(t *testing.T) {
	st := newHandlerTestStorage()
	now := time.Now().UTC()
	st.agents["a1"] = storage.Agent{ID: "a1", Name: "busy", Type: "codex", Status: storage.AgentStatusBusy, CreatedAt: now, UpdatedAt: now}
	app := newTestApp(t, st, &handlerTestQueue{})

	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodPut, "/agent/a1", bytes.NewBufferString(`{"name":"new","type":"codex"}`))
	app.Router().ServeHTTP(rec, req)
...
func TestAgentUpdateAllowsIdle(t *testing.T) {
	st := newHandlerTestStorage()
	now := time.Now().UTC()
	st.agents["a1"] = storage.Agent{ID: "a1", Name: "idle", Type: "codex", Status: storage.AgentStatusIdle, CreatedAt: now, UpdatedAt: now}
	app := newTestApp(t, st, &handlerTestQueue{})

	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodPut, "/agent/a1", bytes.NewBufferString(`{"name":"updated","type":"copilot"}`))
	app.Router().ServeHTTP(rec, req)
