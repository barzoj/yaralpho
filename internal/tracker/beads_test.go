...
func TestBeadsAddCommentInline(t *testing.T) {
    runner := &fakeRunner{}
    b := &Beads{logger: zap.NewNop(), timeout: time.Second, run: runner.run}
    repoPath := t.TempDir()

    if err := b.AddComment(context.Background(), repoPath, "ref-1", "hello world"); err != nil {
    ...
    if runner.dir != repoPath {
        t.Fatalf("dir = %q, want %q", runner.dir, repoPath)
    }
}
...
func TestBeadsAddCommentWithTempFile(t *testing.T) {
    ...
    b := &Beads{logger: zap.NewNop(), timeout: time.Second, run: run}
    repoPath := t.TempDir()

    comment := "hello\nworld"
    if err := b.AddComment(context.Background(), repoPath, "ref-2", comment); err != nil {
        t.Fatalf("AddComment error: %v", err)
    }
    ...
    if capturedDir != repoPath {
        t.Fatalf("dir = %q, want %q", capturedDir, repoPath)
    }
    if body != comment {
...
func TestBeadsAddCommentValidatesRepoPath(t *testing.T) {
    b := &Beads{logger: zap.NewNop(), timeout: time.Second, run: defaultCommandRunner}

    if err := b.AddComment(context.Background(), "", "ref-1", "text"); err == nil {
        t.Fatalf("expected error for empty repo path")
    }

    tempFile, err := os.CreateTemp(t.TempDir(), "file-*")
    if err != nil {
    ...
    if err := b.AddComment(context.Background(), tempFile.Name(), "ref-2", "text"); err == nil {
        t.Fatalf("expected error for non-directory repo path")
    }
}
...
func TestBeadsGetTitleReturnsMatch(t *testing.T) {
    ...
    title, err := b.GetTitle(context.Background(), repoPath, "ref-1")
    if err != nil {
...
func TestBeadsGetTitlePropagatesError(t *testing.T) {
    ...
    if _, err := b.GetTitle(context.Background(), t.TempDir(), "ref-1"); err == nil {
...
func TestBeadsAddCommentPropagatesRunnerError(t *testing.T) {
    ...
    if err := b.AddComment(context.Background(), t.TempDir(), "ref-3", "text"); err == nil {
...
func TestBeadsFetchCommentsParsesOutput(t *testing.T) {
    ...
    comments, err := b.FetchComments(context.Background(), repoPath, "ref-1")
    if err != nil {
...
func TestBeadsFetchCommentsMissingComments(t *testing.T) {
    ...
    comments, err := b.FetchComments(context.Background(), t.TempDir(), "ref-1")
    if err != nil {
        t.Fatalf("FetchComments error: %v", err)
    }
    if len(comments) != 0 {
...
func TestBeadsFetchCommentsPropagatesRunnerError(t *testing.T) {
    ...
    if _, err := b.FetchComments(context.Background(), t.TempDir(), "ref-1"); err == nil {
        t.Fatalf("expected error")
    }
}
